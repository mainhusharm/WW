...
const FUTURES_SERVICE_URL = import.meta.env.DEV ? 'http://localhost:3001' : 'https://backend-qphz.onrender.com';
...
class RealTimePriceSimulator {
  ...
  private async initializeBaseData() {
    // Fetch initial real data from yfinance or our service
    try {
      const symbols = Object.keys(FUTURES_ASSETS);
      const response = await axios.post(`${FUTURES_SERVICE_URL}/api/bulk`, {
        symbols: symbols,
        timeframe: '1m',
        range: '1d'
      });
...
const FuturesDashboard = ({ isBotRunning, setIsBotRunning }: { isBotRunning: boolean, setIsBotRunning: (running: boolean) => void }) => {
  ...
  const fetchFuturesData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setFuturesData(response.data.data);
        setLastUpdate(new Date());
      }
    } catch (error) {
