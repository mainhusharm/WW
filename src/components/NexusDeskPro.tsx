...
import { 
  ...
  Eye, Edit, Trash, Phone, Mail, MapPin, Calendar
} from 'lucide-react';
...
const NexusDeskPro = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch('http://localhost:3005/api/dashboard/notifications');
      if (response.ok) {
    ...
    try {
      ...
      if (customersData.customers) {
        ...
        const serviceDataPromises = customersData.customers.map(async (customer: Customer) => {
          try {
            const response = await fetch(`http://localhost:3005/api/enhanced/customers/${customer.id}/comprehensive`);
            if (response.ok) {
              const data = await response.json();
              return data.data;
            }
        ...
        const ticketsResponse = await fetch('http://localhost:3005/api/dashboard/tickets');
        ...
        const statsResponse = await fetch('http://localhost:3005/api/dashboard/stats');
        if (statsResponse.ok) {
        ...
        const realLiveChats: LiveChat[] = customersData.customers.slice(0, 3).map((customer, index) => ({
          id: index + 1,
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3005/api/customers/search?search=${encodeURIComponent(searchQuery)}`);
      const data = await response.json();
