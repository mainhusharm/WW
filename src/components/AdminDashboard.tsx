...
import { BarChart, Users, RadioTower, Settings, LogOut, Cpu, Send, CheckCircle } from 'lucide-react';
...
const AdminDashboard = ({ onLogout }: { onLogout: () => void }) => {
  ...
  useEffect(() => {
    if (activeTab === 'payment-verification') {
      ...
      setPendingVerifications(allPending.filter((v: any) => v.status === 'pending'));
    }
  }, [activeTab]);
          ...
          <div className="p-6">
            <div className="flex space-x-2 mb-6 border-b border-gray-800">
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold transition-colors rounded-t-lg ${activeTab === 'payment-verification' ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white'}`}
              >
            ...
            {activeTab === 'payment-verification' && (
              <div className="bg-gray-800/60 backdrop-blur-sm p-6 rounded-2xl border border-gray-700 animate-fade-in-up">
                ...
                <p className="text-gray-400 mb-6">
                  Review and approve manual Razorpay payment verifications submitted by users.
                </p>

                <div className="space-y-4">
                  {pendingVerifications.length > 0 ? (
                    pendingVerifications.map((verification, index) => (
                      <div key={index} className="bg-gray-900/50 p-6 rounded-lg border border-yellow-500/30">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              <div>
                                ...
                                <span className="text-white ml-2 font-medium">{verification.plan}</span>
                              </div>
                              <div>
                                <span className="text-gray-400">Amount:</span>
                                <span className="text-white ml-2 font-medium">${verification.amount}</span>
                              </div>
                              <div>
                                <span className="text-gray-400">Payment ID:</span>
                                <span className="text-white ml-2 font-mono text-xs">
                                  {verification.razorpay_payment_id?.substring(0, 15)}...
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">Order ID:</span>
                                <span className="text-white ml-2 font-mono text-xs">
                                  {verification.razorpay_order_id?.substring(0, 15)}...
                                </span>
                              </div>
                              <div>
                              ...
                              {verification.verificationDeadline && (
                                <div className="md:col-span-2">
                                  <span className="text-gray-400">Verification Deadline:</span>
                                  <span className="text-orange-400 ml-2 font-semibold">
                                    {new Date(verification.verificationDeadline).toLocaleString()}
                                  </span>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mb-4">
                          <div className="text-yellow-400 font-semibold mb-2">Payment Details:</div>
                          <div className="text-yellow-300 text-sm space-y-1 font-mono">
                            <div><strong>Payment ID:</strong> {verification.razorpay_payment_id}</div>
                            <div><strong>Order ID:</strong> {verification.razorpay_order_id}</div>
                            <div><strong>Signature:</strong> {verification.razorpay_signature?.substring(0, 30)}...</div>
                          </div>
                        </div>
                        ...
                        <div className="flex space-x-3">
                          <button
                            onClick={() => {
                              ...
                              const allPending = JSON.parse(localStorage.getItem('all_razorpay_manual_pending') || '[]');
                              const updated = allPending.map((v: any) =>
                                v.razorpay_payment_id === verification.razorpay_payment_id
                                  ? { ...v, status: 'verified', verifiedAt: new Date().toISOString() }
                                  : v
                              );
                              localStorage.setItem('all_razorpay_manual_pending', JSON.stringify(updated));

                              // Also store the verified payment separately
                              localStorage.setItem('razorpay_verified_payment', JSON.stringify(verifiedPayment));
                              localStorage.removeItem('payment_verification_pending');

                              // Update user's subscription status
                              const user = JSON.parse(localStorage.getItem('user') || '{}');
                              if (user.id) {
                                user.membershipTier = verification.plan.toLowerCase();
                                localStorage.setItem('user', JSON.stringify(user));
                              }

                              // Refresh the list
                              setPendingVerifications(prevList =>
                                prevList.filter(v => v.razorpay_payment_id !== verification.razorpay_payment_id)
                              );

                              alert('Payment verified successfully! User now has dashboard access.');
                            }}
                          ...
                          <button
                            onClick={() => {
                              ...
                              const allPending = JSON.parse(localStorage.getItem('all_razorpay_manual_pending') || '[]');
                              const updated = allPending.filter((v: any) =>
                                v.razorpay_payment_id !== verification.razorpay_payment_id
                              );
                              localStorage.setItem('all_razorpay_manual_pending', JSON.stringify(updated));
                    ...
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <p className="text-lg">No pending payment verifications.</p>
                      <p className="text-sm mt-2">Manual verifications will appear here when users submit them.</p>
                    </div>
                  )}
                </div>
              </div>
