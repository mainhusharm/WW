    ...
    class ReliabilityManager {
        ...
        async performHealthCheck() {
            try {
                ...
                await this.checkServiceHealth('binance', 'https://binance-service.onrender.com/health');
                await this.checkServiceHealth('forex', 'https://forex-data-service.onrender.com/health');
        ...
        async attemptServiceReconnection(serviceName: string) {
            ...
            try {
                if (serviceName === 'binance') {
                    await this.checkServiceHealth('binance', 'http://localhost:5010/health');
                } else if (serviceName === 'forex') {
                    await this.checkServiceHealth('forex', 'http://localhost:3004/health');
                }
                
                log(`✅ Successfully reconnected to ${serviceName} service`, 'success');
            } catch (error: any) {
                log(`❌ Reconnection to ${serviceName} failed: ${error.message}`, 'error');
            }
        }
