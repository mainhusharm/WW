...
const EnhancedCustomerServiceDashboard = ({ onLogout }: { onLogout?: () => void }) => {
  ...
  useEffect(() => {
    const savedChats = JSON.parse(localStorage.getItem('cs_live_chats') || '[]');
    setLiveChats(savedChats);
  }, []);
  ...
  useEffect(() => {
    const handleNewChat = (event: CustomEvent) => {
      ...
      const newChat: LiveChat = {
        ...
        priority: chatData.priority ? 'high' : 'medium'
      };
                      ...
                      <div className="p-6 border-b border-cyan-500/20">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => {
                                const chatToTransfer = liveChats.find(c => c.chatId === currentChat);
                                if (chatToTransfer) {
                                  const transferredChats = JSON.parse(localStorage.getItem('admin_transferred_chats') || '[]');
                                  transferredChats.push(chatToTransfer);
                                  localStorage.setItem('admin_transferred_chats', JSON.stringify(transferredChats));

                                  const updatedLiveChats = liveChats.filter(c => c.chatId !== currentChat);
                                  setLiveChats(updatedLiveChats);
                                  localStorage.setItem('cs_live_chats', JSON.stringify(updatedLiveChats));
                                  setCurrentChat(null);
                                }
                              }}
