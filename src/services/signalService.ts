...
class SignalService {
  ...
  private async relaySignal(signalRelay: SignalRelay): Promise<void> {
    try {
      // Send to backend signal feed
      const response = await api.post('/api/signals/relay', {
        signal: signalRelay.signal,
        uniqueKey: signalRelay.uniqueKey
      });

      if (response.status === 200) {
        signalRelay.status = 'delivered';
        console.log(`Signal ${signalRelay.signalId} successfully relayed to user feed`);
