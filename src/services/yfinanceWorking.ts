...
class YFinanceWorkingService {
  ...
  private async fetchWithForexDataService(symbol: string, timeframe: string): Promise<Response | null> {
    try {
      const response = await fetch(`https://forex-data-service.onrender.com/api/forex-data?pair=${encodeURIComponent(symbol)}&timeframe=${timeframe}`);
  ...
  async fetchLatestPrice(symbol: string): Promise<YFinancePrice | null> {
    ...
    try {
      const response = await fetch(`https://forex-data-service.onrender.com/api/forex-price?pair=${encodeURIComponent(symbol)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  async fetchBulkData(symbols: string[], timeframe: string): Promise<{ [key: string]: any[] }> {
    try {
      ...
      const response = await fetch(`https://forex-data-service.onrender.com/api/bulk-forex-price?pairs=${encodeURIComponent(symbols.join(','))}`);
...
export const fetchRealTimePrice = async (symbol: string, timeframe: string = '1m', range: string = '1d') => {
  try {
    ...
    // Return mock data as fallback
    const mockData = generateMockPriceData(symbol, timeframe, range);
    console.log(`🔄 Using mock data for ${symbol} due to service failure`);
    return mockData;
  }
};
...
export const fetchBulkPrices = async (symbols: string[], timeframe: string = '1m', range: string = '1d') => {
  try {
    ...
    // Return mock data as fallback
    const mockData = generateMockBulkData(symbols, timeframe, range);
    console.log(`🔄 Using mock data for bulk prices due to service failure`);
    return mockData;
  }
};

// Helper functions to generate mock data
const generateMockHistoricalData = (symbol: string, timeframe: string, range: string) => {
  const count = 100; // Generate 100 bars
  const data = [];
...
const generateMockPriceData = (symbol: string, timeframe: string, range: string) => {
...
const generateMockBulkData = (symbols: string[], timeframe: string, range: string) => {
  const data = symbols.map(symbol => generateMockPriceData(symbol, timeframe, range));
